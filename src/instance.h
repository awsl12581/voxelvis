...
private:
    ...
    const char* frag =
        ...
        "    vec3 finalColor = (ambient + diffuse + specular) * mix(fogColor, "
        "vColor, fogFactor);\n"
    ...
    const char* computed =
        ...
        "if (abs(pos.x) <= pos.w && abs(pos.y) <= pos.w && pos.z <= pos.w && "
        "pos.z >= 0.0) {\n"
