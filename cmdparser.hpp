...
private:
    ...
    static unsigned long long
        parse(const std::vector<std::string>& elements, const unsigned long long&, int numberBase = 0)
    {
        if (elements.size() != 1) {
            throw std::bad_cast();
        }
...
#pragma warning(pop)
    ...
    template <typename T>
    ArgumentPromise<T> set_required(
        const std::string& name,
        const std::string& alternative,
        const std::string& description = "",
        bool dominant = false)
    ...
    template <typename T>
    ArgumentPromise<T> set_optional(
        const std::string& name,
        const std::string& alternative,
        T defaultValue,
        const std::string& description = "",
...
protected:
    ...
    void set_general_help_text(const std::string& generalHelpText) { _general_help_text = generalHelpText; }
